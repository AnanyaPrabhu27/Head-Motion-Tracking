...
public class ZED3DObjectVisualizer : MonoBehaviour
{
    ...
    [Space(5)]
    [Header("Filters")]
    // Single-target view toggle (added)
    [Header("Single Target View")]
    [Tooltip("If true, only draw the bounding box of the selected ID.")]
    public bool onlyShowSelectedBox = false;

    // Use -1 to indicate 'no selection'. Set by SingleTargetDetection at runtime.
    ...
    private void Visualize3DBoundingBoxes(ObjectDetectionFrame dframe)
    {
        ...
        // Filter to a single selected object if requested
        if (onlyShowSelectedBox && selectedId >= 0)
        {
            newobjects = newobjects.Where(o => o.id == selectedId).ToList();
        }
    ...
    private void Visualize3DBoundingBoxes(BodyTrackingFrame dframe)
    {
        ...
        if (onlyShowSelectedBox && selectedId >= 0)
        {
            newbodies = newbodies.Where(b => b.id == selectedId).ToList();
        }
